...
{
	return(HAL_I2C_Master_Transmit(AHT20_I2C_BUS_HANDLE, AHT20_ADDR, command_buffer, size, HAL_MAX_DELAY));
}

HAL_StatusTypeDef Get_AHT20_Data(uint8_t * data_buffer, size_t size)
{
	return(HAL_I2C_Master_Receive(AHT20_I2C_BUS_HANDLE, AHT20_ADDR, data_buffer, size, HAL_MAX_DELAY));
}
